...
int $$_Hash::resize($1 *hp, int newSz){
    ...
    if($$_Array::sizeChange(hp,newSz,2))return 1; // 2=exact size, ret 0 for OK
...
$2* $$_Hash::get($1 *hp, $2 *obj){
	...
	printf("$$_Hash:get() cannot find the object\n");
	return NULL;
    }
    p=($2*)(hp->$0.array[i]);
    if(p){
...
int $$_Hash::add($1 *hp, $2 *obj){ // obj = object to load into the hash table
    ...
    if(i<0 || i>=hp->$0.sz){
	printf("hashing error bucket=%d for size=%d\n",i,hp->$0.sz);
	printf("object not added to the $$_Hash\n");
	return 1;
    }
    p=($2*)(hp->$0.array[i]);
